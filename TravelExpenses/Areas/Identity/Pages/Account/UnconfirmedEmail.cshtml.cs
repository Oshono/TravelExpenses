            ...
            if (ModelState.IsValid)
            {
                ...
                if (user.Email != Input.Email)
                {
                    ...
                    if (_userManager.Options.User.RequireUniqueEmail)
                    {
                        ...
                        {
                            ModelState.AddModelError(string.Empty, new IdentityErrorDescriber().DuplicateEmail(Input.Email).Description);
                            return Page();
                        }
                    }
                ...
                await _emailSender.SendEmailAsync(Input.Email, "Confirma tu correo",
                    $"Por favor confirma tu correo <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>Click aqui.</a>.");
